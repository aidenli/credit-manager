...
| OAuth 测试 | 每个可用 OAuth 账号一张卡片：每天定时或手动问固定两道题（题1 纯文本、题2 生成 SVG 动画 HTML），卡片内直接看结果 |

### OAuth 智商测试

插件按顺序对**每个可用 OAuth 账号**问同一组问题，用来横向比较账号"答得怎么样"。

- **题1**：`你最后的训练数据，日本首相是谁?不允许搜索` —— 只保存文本答案，不自动判分。
- **题2**：`创建一个HTML，内容是SVG绘制一个鹈鹕骑自行车的2D动画，你不需要任何测试。` —— 插件先从回答里**提取 HTML**（优先代码块，其次完整 `<!DOCTYPE html>…</html>`，再退到裸 `<svg>…</svg>`；都提取不到就原样保留，让你看到模型究竟回了什么），把提取结果**写成 `.html` 文件**存到数据目录的 `oauth-tests/` 下（卡片上显示路径，也可点「下载 HTML」），并在卡片结果区以 blob URL 作为**真实文档运行预览**（`sandbox="allow-scripts"`：允许脚本，但不给 `allow-same-origin`，所以文档处在**不透明源**，读不到控制台的 DOM、cookie、localStorage；代价是模型输出的 JS 可以发网络请求——换来 JS 动画和模型自带的暂停按钮都能用）。CSS/SMIL 动画到处都能播。题2 不做内容校验，不会因为"没找到 `<svg>`"就把账号判失败。

两题都是固定的，没有自定义提示词：控制台要横向比较账号，每个账号必须被问同一件事。

三条设计约束值得记住：

1. **不使用兜底**：探针通过 warmup 会话头把请求钉死在被测账号上，调度器要么返回这个账号、要么拒绝，因此"失败"就是失败，不会被别的账号顶上。**所有账号同时开跑**，某个账号失败或返回为空都只**记录**在该账号名下，不会中断其他账号。如果账号因为并发已满而拒绝，探针会**等一会儿再问**（15 秒一次，直到该题 10 分钟预算用完）；只有等到预算耗尽才记为该账号失败。
2. **结果按账号落库**：每测完一个账号就写一次，某个账号单独测试也只替换它自己那张卡片；服务重启后卡片上的最近结果仍在。卡片上直接写出该账号的**测试时间**（按你浏览器本地时间显示）和**本次耗时**。测试进行中，控制台每 5 秒**只查询还没出结果的账号**并原地替换这些卡片，不会整页重刷（否则每次都会重载所有预览）。
3. **不计账但耗额度**：探针不建预留、不进用量账本、不占 Key 额度；但它确实会消耗该 OAuth 账号的上游额度（两道题各一次调用，每题最多 10 分钟）。

调度：工具栏里勾选**启用每日定时**并选择运行时间（按**浏览器本地时间**显示，保存时换算成 UTC 存储，因此不受服务器时区影响），点「保存设置」生效；不开启定时时，只有「测试全部账号」或卡片上的「测试」会发起调用。定时默认**关闭**，改设置会立即重建定时器，但不会打断正在跑的那一批。
...
兜底请求的并发记账：key 的 `max_concurrent_requests` 与"当前并发量"**只统计由该 key 自己账号承载的在途请求**。额度打满时，插件不再直接拒绝，而是**溢出到兜底**（若兜底已启用且当前候选里有 API 提供商），因此该上限的含义是"最多 N 个请求由自己的账号承载，超出的走付费兜底"；兜底不可用时才会返回原来的并发拒绝。认证并发按本次尝试实际派发的凭证计，兜底尝试计在 API 提供商上，不会占用绑定 OAuth 账号的并发。
...
| GET | `/oauth-tests` | 账号卡片状态：定时设置 + 每个可用 OAuth 账号的最近结果与是否在测试中；带 `?auth_ids=a,b` 时只返回这些账号（控制台轮询未完成账号用） |
| POST | `/oauth-tests/settings` | 保存每日定时（`enabled`、`hour_utc`、`minute_utc`）与模型、思考强度 |
| POST | `/oauth-tests/run` | 立即测试：空 body 测全部账号，`{"auth_id":"..."}` 只测一个 |
| POST | `/oauth-tests/stop` | 停止正在进行的测试（不影响每日定时） |
