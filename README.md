...
| OAuth 测试 | 定时或手动轮询所有可用 OAuth 账号，问同样两道题（题1 纯文本、题2 生成 SVG 动画 HTML），并预览最近一次结果 |

### OAuth 智商测试

插件按顺序对**每个可用 OAuth 账号**问同一组问题，用来横向比较账号"答得怎么样"。

- **题1**：`你最后的训练数据，日本首相是谁?不允许搜索` —— 只保存文本答案，不自动判分（控制台里那栏叫「题1 自定义提示词」，填了就替换题1）。
- **题2**：`创建一个HTML，内容是SVG绘制一个鹈鹕骑自行车的2D动画，你不需要任何测试。` —— 保存模型返回的 HTML 原文，控制台用 `iframe sandbox=""` 预览（**禁用脚本**，因为这段 HTML 是模型输出、而控制台是特权页面）；SMIL/CSS 动画能正常播放，纯 JS 动画会是静止的。题2 不做内容校验，不会因为"没找到 `<svg>`"就把账号判失败。
...
2. **结果逐条落库**：每测完一个账号就写一次，所以一轮要跑好几分钟时控制台能看到进度；服务重启会把遗留的 `running` 标成中断。
3. **不计账但耗额度**：探针不建预留、不进用量账本、不占 Key 额度；但它确实会消耗该 OAuth 账号的上游额度（两道题各一次调用，每题最多 10 分钟，单账号上限 21 分钟）。

定时开关默认**关闭**，间隔默认 60 分钟（1–1440 可调）。开启后按间隔自动跑一轮；改设置会立即重建定时器，但不会打断正在跑的那一轮。
...
| GET / POST | `/oauth-tests/settings` | 读取或保存 OAuth 测试的定时开关、间隔、模型、思考强度与自定义题1 |
| GET | `/oauth-tests/latest` | 最近一次测试运行（含每个账号的题1文本与题2 HTML） |
| POST | `/oauth-tests/run` | 立即开始一轮测试（进行中会返回冲突） |
| POST | `/oauth-tests/stop` | 停止进行中的一轮测试（不影响定时） |
