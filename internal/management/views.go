...
func keyView(key store.PluginKey) map[string]any {
	...
	return map[string]any{
		...
		"total_spend_reset_at":    key.TotalSpendResetAt,
		"daily_spend_reset_at":    key.DailySpendResetAt,
		"weekly_spend_reset_at":   key.WeeklySpendResetAt,
		"monthly_spend_reset_at":  key.MonthlySpendResetAt,
...
func usageView(entry store.UsageEntry) map[string]any {
	return map[string]any{
		...
		"served_api":               entry.ServedAPI,
		"served_provider":          entry.ServedProvider,
		"model":                    entry.Model,
