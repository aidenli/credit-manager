...
(() => {
  const $ = id => document.getElementById(id), endpoint = location.pathname.replace(/\/lookup\/?$/, '/lookup/data'); let token = '', data = null, modelMetric = 'requests', modelRankMetric = 'value', tokenTrendGrain = 'hour', costTrendGrain = 'hour', recentPage = 1, recentPageSize = 10; const charts = {};
  ...
  const COPY = {
    ...
    '到期': { 'zh-TW':'到期', en:'expires', ru:'до' },
    '已于': { 'zh-TW':'已於', en:'Expired', ru:'Истёк' },
    '过期': { 'zh-TW':'過期', en:'expired', ru:'истёк' },
    '还有': { 'zh-TW':'還有', en:'in', ru:'через' },
    '小时': { 'zh-TW':'小時', en:'hours', ru:'часов' },
    '天': { 'zh-TW':'天', en:'days', ru:'дн.' },
    '不到 1 小时后到期': { 'zh-TW':'不到 1 小時後到期', en:'Expires in under 1 hour', ru:'Истекает меньше чем через час' },
  ...
  function renderModelTokenLimits(result) {
    ...
    const cards = items.map(item => '<div class="quota-card token-limit-card"><div class="quota-top"><span class="quota-name" title="'+esc(item.model)+'">'+esc(item.model)+'</span></div><div class="token-period-grid">'+tokenPeriodBar('日 Token', item.daily_used, item.daily)+tokenPeriodBar('周 Token', item.weekly_used, item.weekly)+tokenPeriodBar('月 Token', item.monthly_used, item.monthly)+tokenPeriodBar('总 Token', item.total_used, item.total)+'</div></div>');
